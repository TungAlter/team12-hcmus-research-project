...
```
```
Error: no member named 'Fly' in 'PlasticToyDuck'
```
...
```c++
struct WeatherFinder : public IWetherFinder
{
...
struct WeatherAdapter : public WeatherFinderClient
{
    double getTemperature(String zipcode)
    {
        String cityName = getCityName(zipcode);
        WetherFinder wetherFinder = new WeatherFinder();
